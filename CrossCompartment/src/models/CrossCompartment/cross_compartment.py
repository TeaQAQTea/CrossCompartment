...
try:
    ...
    class SambaMLP(nn.Module):
        def __init__(self, hidden_size, hidden_ratio=4.0, hidden_act="swish", fuse_swiglu=True):
            super().__init__()
            hidden = int(hidden_size * hidden_ratio)
            self.fc1 = nn.Linear(hidden_size, hidden)
            self.fc2 = nn.Linear(hidden, hidden_size)
            self.hidden_act = hidden_act

        def forward(self, x, **kwargs):
            if self.hidden_act in {"swish", "silu"}:
                x = F.silu(self.fc1(x))
            else:
                x = F.gelu(self.fc1(x))
            return self.fc2(x)
    ...
    class Attention(nn.Module):
        ...
        def forward(self, hidden_states, past_key_values=None, **kwargs):
            out, weights = self.attn(hidden_states, hidden_states, hidden_states, need_weights=False)
            return out, weights, past_key_values
    ...
    if comba is None:
        class FallbackComba(nn.Module):
            def forward(self, x, *args, **kwargs):
                return x
        return lambda *args, **kwargs: FallbackComba()
